...
test("existing account: approve then deposit, spender is the base asset contract", () => {
  let state = startDepositFlow({ addresses, amountUnits: 100_000_000n, subaccountId: "42", walletAddress: wallet });
...
test("sufficient allowance skips the approval step", () => {
  let state = startDepositFlow({ addresses, amountUnits: 5n, subaccountId: "42", walletAddress: wallet });
  state = transitionDepositFlow(state, { preflight: preflight({ allowance: 10n }), type: "PREFLIGHT_RESOLVED" });
  expect(state.status).toBe("awaiting-deposit");
});

test("create path: spender is the creator and confirmation must carry the new subaccount id", () => {
  let state = startDepositFlow({ addresses, amountUnits: 100n, subaccountId: null, walletAddress: wallet });
  ...
  state = transitionDepositFlow(state, { preflight: preflight({ allowance: 100n }), type: "PREFLIGHT_RESOLVED" });
  expect(state.status).toBe("awaiting-deposit");
...
test("whitelist enabled blocks the create path and non-whitelisted accounts", () => {
  let createState = startDepositFlow({ addresses, amountUnits: 100n, subaccountId: null, walletAddress: wallet });
  ...
  let existingState = startDepositFlow({ addresses, amountUnits: 100n, subaccountId: "42", walletAddress: wallet });
...
test("blocks zero amounts and insufficient balance", () => {
  const zeroState = startDepositFlow({ addresses, amountUnits: 0n, subaccountId: "42", walletAddress: wallet });
  expect(zeroState.status).toBe("blocked");
  expect(zeroState.reason).toBe("zero-amount");

  let poorState = startDepositFlow({ addresses, amountUnits: 100n, subaccountId: "42", walletAddress: wallet });
...
test("errors record the failing step and retry resumes from it", () => {
  let state = startDepositFlow({ addresses, amountUnits: 100n, subaccountId: "42", walletAddress: wallet });
...
test("irrelevant events do not change state", () => {
  const state = startDepositFlow({ addresses, amountUnits: 100n, subaccountId: "42", walletAddress: wallet });
