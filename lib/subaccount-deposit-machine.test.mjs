...
test("create path: spender is the creator and confirmation must carry the new subaccount id", () => {
  ...
  // No receipt block reported, so balance refreshes fall back to a plain read.
  expect(state.blockNumber).toBeNull();
});

/**
 * The receipt's block rides into success so the terminal can re-read balances at or past it. Without
 * it the refresh read latest, hit a node a block behind, and showed the pre-deposit balance.
 */
test("a confirmed deposit carries its receipt block", () => {
  let state = startDepositFlow({
    addresses,
    amountUnits: 5n,
    subaccountId: "19",
  ...
  state = transitionDepositFlow(state, { txHash: "0xddd", type: "DEPOSIT_SUBMITTED" });
  state = transitionDepositFlow(state, { blockNumber: 51_263_588n, type: "DEPOSIT_CONFIRMED" });

  expect(state.status).toBe("success");
  expect(state.blockNumber).toBe(51_263_588n);
});
