...
test("change tracks the live last price, not just the last bucket", () => {
  const candles = [candle(1, { close: 1010, open: 1000 })];
...
test("high and low span the same 24h window as volume and change", () => {
  const stats = get24hStats(
    [
      candle(40, { high: 9999, low: 1 }), // outside the window — must not set either extreme
      candle(6, { high: 1380, low: 1360 }),
      candle(2, { high: 1375, low: 1350 }),
    ],
    1371.98,
    NOW
  );
  ...
  expect(stats.low).toBe(1350);
});

test("an empty window reports no high or low", () => {
  const stats = get24hStats([candle(48)], 1371.98, NOW);
