...
import { presentStats24h } from "./spot-market.ts";
import { get24hStats, getVenueLastPrice, lastTradeAgeLabel } from "./ticker-stats.ts";
...
test("a recent trade needs no age suffix", () => {
  const now = new Date("2026-09-14T12:00:00Z");
  expect(lastTradeAgeLabel([{ timestamp: "2026-09-14T11:30:00Z" }], now)).toBe(null);
});

test("a trade older than the window is marked with its age", () => {
  const now = new Date("2026-09-14T12:00:00Z");
  expect(lastTradeAgeLabel([{ timestamp: "2026-09-14T08:00:00Z" }], now)).toBe("4h ago");
});

// The case on screen: a fill from two days ago presented as the current price, beside a live book
// quoting fourteen naira away. TradePrint.time is HH:MM with no date, so nothing else disambiguated it.
test("a days-old trade says so in days", () => {
  const now = new Date("2026-09-14T12:00:00Z");
  expect(lastTradeAgeLabel([{ timestamp: "2026-09-12T02:32:54Z" }], now)).toBe("2d ago");
});

// A clock a little ahead of the venue must not render an age in the future.
test("a timestamp slightly in the future is not stale", () => {
  const now = new Date("2026-09-14T12:00:00Z");
  expect(lastTradeAgeLabel([{ timestamp: "2026-09-14T12:00:30Z" }], now)).toBe(null);
});

test("no trade, or an unparseable one, has no age", () => {
  expect(lastTradeAgeLabel([])).toBe(null);
  expect(lastTradeAgeLabel([{}])).toBe(null);
  expect(lastTradeAgeLabel([{ timestamp: "not-a-date" }])).toBe(null);
});
