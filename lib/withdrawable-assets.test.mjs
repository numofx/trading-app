 ...
 */
test("mainnet offers both USDC escrows plus cNGN, the settling one first", () => {
  withChain(BASE_MAINNET_CHAIN_ID, () => {
    const assets = getWithdrawableAssets();

    expect(assets.map((asset) => asset.id)).toEqual(["usdc-wrapped", "usdc-cash", "cngn"]);
    expect(assets[0].escrow).toBe("0x364058aFF6f36E01505fB2Cc870f8B6BD4835e84");
    expect(assets[1].escrow).toBe("0x6B232A2155Bd0C9bf741dB4cf8E7e8A0176A6fc6");
...
test("the two USDC escrows share one token", () => {
  withChain(BASE_MAINNET_CHAIN_ID, () => {
    const [wrapped, cash] = getWithdrawableAssets();
...
test("assets are addressable by id", () => {
  withChain(BASE_MAINNET_CHAIN_ID, () => {
    expect(findWithdrawableAsset("usdc-cash")?.label).toBe("Legacy USDC");
...
test("no sibling is offered across tickers, or when it holds nothing", () => {
  withChain(BASE_MAINNET_CHAIN_ID, () => {
    ...
    expect(
      findSiblingAssetWithBalance({ assets, current: assets[1], rows: emptyWrapped })
    ).toBeNull();
  });
});
