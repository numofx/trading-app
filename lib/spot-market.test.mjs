 ...
 */
const ASKS = [
  { price: 1400, size: 10, total: 10 },
  { price: 1410, size: 10, total: 20 },
  { price: 1450, size: 30, total: 50 },
];
const BIDS = [
  { price: 1390, size: 5, total: 5 },
  { price: 1380, size: 15, total: 20 },
];

test("a buy inside the touch fills at the touch", () => {
  const fill = getMarketFill("buy", ASKS, BIDS, 5);
  expect(fill.averagePrice).toBeCloseTo(1400, 6);
  expect(fill.filledSize).toBeCloseTo(5, 6);
  expect(fill.isFullyFilled).toBe(true);
});

test("a buy through two levels averages them by size", () => {
  // 10 @ 1400 + 5 @ 1410 = 21,050 over 15 USDC.
  const fill = getMarketFill("buy", ASKS, BIDS, 15);
  expect(fill.averagePrice).toBeCloseTo(21_050 / 15, 6);
...
test("a sell walks the bids down, not the asks up", () => {
  // 5 @ 1390 + 5 @ 1380 = 13,850 over 10 USDC.
  const fill = getMarketFill("sell", ASKS, BIDS, 10);
  expect(fill.averagePrice).toBeCloseTo(13_850 / 10, 6);
  expect(fill.averagePrice).toBeLessThan(BIDS[0].price);
});

test("an order past the book reports what the depth covers", () => {
  const fill = getMarketFill("sell", ASKS, BIDS, 100);
  expect(fill.filledSize).toBeCloseTo(20, 6);
  expect(fill.isFullyFilled).toBe(false);
  // Still a real average over what rests — the remainder rests rather than filling.
  // 5 @ 1390 + 15 @ 1380 = 27,650 over 20 USDC.
  expect(fill.averagePrice).toBeCloseTo(27_650 / 20, 6);
});

test("an empty side prices nothing rather than guessing", () => {
  const fill = getMarketFill("buy", [], BIDS, 5);
...
/*
 * The Amount field can be counted in either leg, but only the USDC figure is submittable: the
 * signed envelope carries no other unit. A cNGN entry converted at the wrong end of the division
 * would size the order by a factor of the price — ~1,400x on this pair.
 */
test("a USDC entry is already the order size", () => {
  expect(toOrderSizeUsdc(100, "USDC", 1400)).toBeCloseTo(100, 6);
});

test("a cNGN entry is divided by the price, not multiplied", () => {
  expect(toOrderSizeUsdc(140_000, "cNGN", 1400)).toBeCloseTo(100, 6);
});

test("a cNGN entry with no price to convert at is not a size", () => {
  expect(Number.isNaN(toOrderSizeUsdc(140_000, "cNGN", null))).toBe(true);
  expect(Number.isNaN(toOrderSizeUsdc(140_000, "cNGN", 0))).toBe(true);
  // A USDC entry needs no price, so it survives an unpriced book.
  expect(toOrderSizeUsdc(100, "USDC", null)).toBeCloseTo(100, 6);
});
...
// --- a market order's size is counted at its expected fill ---

/**
 * Trades #341 and #342: a market buy of 1 USDC signed through an ask of 1339.479393 was sized at the
 * signed ~1346.18 and spent 1,346 cNGN for 1.0049 USDC. The size belongs at the expected fill.
 */
test("a market order's size is counted at its expected fill, not its signed limit", () => {
  const buyLimit = getMarketableLimitPrice("buy", 1339.479_393, null);
  const sellLimit = getMarketableLimitPrice("sell", null, 1336.808_099);

  expect(getMarketSizingPrice("buy", 1339.479_393, buyLimit)).toBe(1339.479_393);
  expect(getMarketSizingPrice("sell", 1336.808_099, sellLimit)).toBe(1336.808_099);
});

/** A size counted past the limit is the inflated amount itself, so the limit is the most it gets. */
test("an expected fill past the signed limit is held at the limit", () => {
  expect(getMarketSizingPrice("buy", 1400, 1346)).toBe(1346);
  expect(getMarketSizingPrice("sell", 1300, 1330)).toBe(1330);
});

test("no limit or no expected fill sizes nothing, rather than falling back to the limit", () => {
  expect(getMarketSizingPrice("buy", 1339, null)).toBeNull();
  expect(getMarketSizingPrice("buy", null, 1346)).toBeNull();
  expect(getMarketSizingPrice("sell", Number.NaN, 1330)).toBeNull();
});

// --- an order that would trade against the trader's own resting order ---

/** The 2026-09-13 pair on account 19: a resting sell at 1,346.18, then a buy signed at the same price. */
...
test("a buy at the price of the trader's own resting sell would cross it", () => {
  expect(
    findOwnCrossingOrder({ ownOrders: [OWN_RESTING_SELL], side: "buy", signedPrice: 1346.18 })
  ).toBe(OWN_RESTING_SELL);
});

test("a buy below the trader's own resting sell does not cross it", () => {
  expect(
    findOwnCrossingOrder({ ownOrders: [OWN_RESTING_SELL], side: "buy", signedPrice: 1346.17 })
  ).toBeNull();
});
...
test("a sell crosses the trader's own resting buy at or above its price, never an own sell", () => {
  ...
  expect(findOwnCrossingOrder({ ownOrders: [ownBuy], side: "sell", signedPrice: 1346.18 })).toBe(ownBuy);
  expect(findOwnCrossingOrder({ ownOrders: [ownBuy], side: "sell", signedPrice: 1346.19 })).toBeNull();
  expect(
    findOwnCrossingOrder({ ownOrders: [OWN_RESTING_SELL], side: "sell", signedPrice: 1300 })
  ).toBeNull();
});
...
test("the nearest crossed order is the one reported", () => {
  const higher = { ...OWN_RESTING_SELL, nonce: "2", orderId: "spot-higher", price: 1350 };

  expect(
    findOwnCrossingOrder({ ownOrders: [higher, OWN_RESTING_SELL], side: "buy", signedPrice: 1353 })
  ).toBe(OWN_RESTING_SELL);
});
...
test("venue stats are presented in trader terms, the inversion swapping high and low", () => {
  const stats = presentStats24h({ orderEntrySpec: "usdc_cngn_spot_v1", stats: VENUE_STATS });

  // The engine's lowest USDC-per-cNGN (trade #344) is the highest cNGN-per-USDC.
  expect(stats.high).toBeCloseTo(1344.83, 2);
  expect(stats.low).toBeCloseTo(1327.12, 2);
  // First price = last - change = trade #341's 0.000746555911576301, inverted.
  expect(stats.firstPrice).toBeCloseTo(1339.48, 1);
...
test("a quiet window reports nothing rather than zeros", () => {
  ...
  expect(presentStats24h({ orderEntrySpec: "usdc_cngn_spot_v1", stats: quiet })).toEqual({
    firstPrice: null,
    high: null,
    low: null,
    quoteVolume: null,
...
test("a service that predates quote_volume leaves the volume unknown, never the cNGN figure", () => {
  ...
  const stats = presentStats24h({ orderEntrySpec: "usdc_cngn_spot_v1", stats: older });

  expect(stats.quoteVolume).toBeNull();
  expect(stats.high).toBeCloseTo(1344.83, 2);
});

test("under any other contract the venue's prices are presented as served", () => {
  const stats = presentStats24h({ orderEntrySpec: null, stats: VENUE_STATS });
...
test("non-positive or unparseable stats are unknown", () => {
  ...
  expect(presentStats24h({ orderEntrySpec: "usdc_cngn_spot_v1", stats: broken })).toEqual({
    firstPrice: null,
    high: null,
    low: null,
    quoteVolume: null,
...
test("no stats from the venue means none on the market", () => {
  ...
  expect(
    buildSpotMarket({
      book: SPOT_BOOK,
      stats24h: { orderEntrySpec: "usdc_cngn_spot_v1", stats: VENUE_STATS },
      trades: [],
    }).stats24h.high
  ).toBeCloseTo(1344.83, 2);
});

/*
 * The bug, from the Open Orders tab on 2026-09-14: a 0.995 USDC buy read "1,307 USDC" in the Filled
 * column. `filled_amount` is the engine cNGN amount, `ui_intent.size` is USDC notional, and
 * collectOpenOrders put both in one object for the view to print with a USDC label on each.
 *
 * Verified against the live book: desired_amount * limit_price == ui_intent.size on every resting
 * order, and trade #348 settled 1307 engine cNGN at 0.000753388839409405 = 0.984679 USDC.
 */
test("filled is the USDC notional, not the raw engine amount", () => {
  const orders = collectOpenOrders({
    bids: [],
    asks: [
      {
        ...
        side: "sell",
        spot_contract: {
          ui_intent: { price: "1327.34", side: "buy", size: "0.999909" },
        },
      },
    ],
  });
  ...
  expect(orders).toHaveLength(1);
  expect(orders[0].filled).toBeCloseTo(0.984_679, 6);
  // The number that was rendered before: the engine amount, three orders of magnitude out.
  expect(orders[0].filled).not.toBeCloseTo(1307, 0);
});

// The second symptom of the same mix-up: `size - filled` subtracted cNGN from USDC, so a
// mostly-filled order still reserved almost its whole cost against the account's balance.
test("a mostly filled order only reserves what is still working", () => {
  ...
  expect(remaining).toBeCloseTo(0.100_363, 6);
  // A tenth of a USDC still working, so roughly a tenth of the cost still committed. Before the
  // fix `filled` was 1194 (engine cNGN), so `size - filled` went negative and committed nothing.
  expect(getCommittedBalances(orders, "0xAAA").cngn).toBeCloseTo(133.2155, 3);
});

// An order with no ui_intent used to be rendered from its engine fields: a ₦0.0007 price and a
// 1,374 USDC size. Dropped instead — but never silently, or a missing row looks like no order.
